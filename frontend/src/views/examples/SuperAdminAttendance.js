...
const SuperAdminAttendance = () => {
  ...
  const fetchStudentsForTeacher = async (teacherId) => {
    ...
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/users/assigned-students/${teacherId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleMarkAttendance = async () => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/api/attendance/mark`, attendanceData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSaveEdit = async (recordId) => {
    ...
    try {
      await axios.put(
        `${process.env.REACT_APP_API_URL}/api/attendance/update/${recordId}`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
